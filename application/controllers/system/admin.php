...
class Admin extends MY_Controller{
	...
	public function index(){
		...
		$this->load->view('system/admin/index', $data);
	}
	...
	public function site_config(){
		...
		$this->load->view('system/admin/site_config', $data);
	...
	public function site_config_add(){
		...
		if($this->form_validation->run() === FALSE){
			$this->load->view('system/admin/site_config_add', $data);
		}else{
			$configData = array(
			...
			if(count($checkVarData) > 0){
				util::showMessage('已有名为' . $checkVarData['var_name'] . '变量的设置！', site_url('system/admin/site_config'));
				exit;
			}
			...
			if(count($checkTtitleData) > 0){
				util::showMessage('已有名为' . $configData['title'] . '名称的设置！', site_url('system/admin/site_config'));
				exit;
			}
			
			
			$this->base_model->addData('site_config', $configData);
			util::showMessage('添加成功！', site_url('system/admin/site_config'));
		}
	}
	...
	public function site_config_edit(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$data['configData'] = $configData;
			$data['id'] = $id;
			$this->load->view('system/admin/site_config_edit', $data);
		}else{
			$configUpdata = array(
				'var_name' => trim($this->input->post('var_name')),
				'title' => trim($this->input->post('title')),
			...
			util::showMessage('修改成功！', site_url('system/admin/site_config'));
		}
	}
	...
	public function base_config(){
		...
		if($this->form_validation->run() === FALSE){			
			...
			$this->load->view('system/admin/base_config', $data);
		}else{
			//循环获取表单值
			foreach($base_config_data as $val){
	...
	public function email_config(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$this->load->view('system/admin/email_config', $data);
		}else{
			//循环获取表单值
			foreach($email_config_data as $val){
				$updata[$val['var_name']]['value'] = $this->input->post($val['var_name']);

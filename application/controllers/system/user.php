...
class User extends MY_Controller{
	...
	public function index(){
		...
		$userCount = $this->base_model->getCount('user');
		$config['base_url'] = site_url('system/user/index');
		...
		$userRow = $this->base_model->getData('user', $where, '', 'id desc', $limit, $offset);
		$roleData = $this->base_model->getData('role');
		...
		$this->load->view('system/user/user_list', $data);
	}
	...
	public function add(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$this->load->view('system/user/user_add', $data);
		}else{
			$userData = array(
			...
			$checkEmail = $this->base_model->getData('user', $email_where);
			if(count($checkEmail)){
				$error_message .= '邮箱：' . $userData['email'] .'已占用\n';
			}
			
			$name_where = "name = '" . $userData['name'] . "'";
			$checkName = $this->base_model->getdata('user', $name_where);
			if(count($checkName)){
			...
			$this->base_model->addData('user', $userData);
			util::showMessage('添加成功!', site_url('system/user'));
		}
	}
	...
	public function edit(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$where = "id = '" . $id . "'";
			$userRow = $this->base_model->getDataRow('user', $where);
			$roleData = $this->base_model->getData('role');
			
			$data['userRow'] = $userRow;
			$data['roleData'] = $roleData;
			$data['id'] = $id;
			$this->load->view('system/user/user_edit.php', $data);
		}else{
			...
			$checkWhere = "id = '" . $id . "'";
			$checkUser = $this->base_model->getDataRow('user', $checkWhere);
			
			if($checkUser['password'] == $oldpassword){
				$this->base_model->editData('user', $checkWhere, $userData);
				util::showMessage('用户修改成功!', site_url('system/user'));
			}else{
				util::showMessage('用户密码与原始密码不符，请重新输入', site_url('system/user/edit/' . $id));
			}
		}
	}
	...
	public function delete(){
		...
		if($userRow['name'] == 'admin' && $userRow['role'] == 0){
			...
			$this->base_model->delData('user', $where);
	...
	public function all_delete(){
		...
		$ids = implode(',' , $ids);
		$where = "id in(" . $ids . ")";
		$userRow = $this->base_model->getData('user', $where);
